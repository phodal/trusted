...
![架构层级](images/architecture-level.jpg)
...
**测试金字塔**，测试分为：单元测试、集成测试、组件测试、端到端测试。
...
> 事件风暴(Event Storming)是一种用于领域驱动设计(Domain Driven Design)的协作设计(Collaborative Design) 方法。旨在通过领域事件识别出聚合根，进而划分微服务的限界上下文。

今天，全景事件风暴已经被证明是一个非常有效的划分限界上下文的方式。

![Event Stroming](images/event-storming.png)
...
个人能力（《精益软件度量》：
 ...
 - 技术能力：GTD。实践有团队技能图谱、
 - 首创能力：产生想法，将实现的能力。主动力，
 - 社交能力：协作过程中发挥其技术能力的能力，诸如协作、沟通、主动、辅导、领导力。
